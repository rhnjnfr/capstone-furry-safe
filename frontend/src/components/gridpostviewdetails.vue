                        ...
                        <DialogPanel
                            ...
                            <div class="flex flex-col bg-white sm:rounded-2xl md:rounded-1x1">
                                <div class="flex sm:flex-col md:flex-col">
                                    ...
                                    <div
                                        class="flex items-center justify-between sm:hidden md:flex gap-x-2 border-b p-[1.5rem] rounded-t-2xl">
                                        <!-- <div> px-[2rem] pb-4 px-[2rem] pb-4
                                                <h3 class="text-base font-semibold leading-7 text-gray-900">
                                                </h3>
                                            </div> -->

                                        <div class="flex items-center justify-between sm:hidden md:flex gap-x-2 ">
                                            <div v-if="userdetails[0]" class="flex items-center gap-x-3">
                                                <img :src="userdetails[0].profile_url || default_profile" alt="profile"
                                                    class="w-11 h-11 rounded-full object-cover " />
                                                <span class="font-medium sm:text-base xl:text-2xl">{{
                                                    selectedPostDetails.posted_by }}'s Post</span>
                                            </div>
                                        </div>

                                        <!-- dropdown buttons -->
                                        <div class="relative inline-block text-left" v-if="localid == _user_id">
                                    ...
                                    <div class="px-4">
                                        <div
                                            class="relative bg-black flex justify-center items-center sm:w-full rounded-2xl group">
                                            <!-- Left button -->
                                            <div
                                            ...
                                            <div class="flex">
                                                <img :src="selectedPostDetails.photos[0]" alt="Image post"
                                                    class="flex-shrink-0 aspect-auto sm:w-full xl:w-[80rem] xl:h-[50rem] object-contain hover:blur-sm" />
                                            </div>
                                            ...
                                            <!-- Middle Button -->
                                            <div
                                                class="absolute z-20 bg-white bg-opacity-60 text-black py-2 px-4 rounded-2xl hidden group-hover:block ">
                                                <button class="text-[20px]" @click="toggleModalViewPostDetails(id)">
                                                    Visit Profile
                                                    <viewpostdetials v-if="selectedPostViewDetailsId === id" :selectedPostDetails="selectedPostDetails"
                                                        @close="toggleModalViewPostDetails(id)" />
                                                </button>
                                            </div>
                                        </div>
                                    </div>
...
// Nov12
const localid = localStorage.getItem('u_id')
let date = ref(null)
let id = ref(null)
onMounted(async () => {
    ...
    id.value = selectedPostDetails.value.post_id
